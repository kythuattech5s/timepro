...

{
    ...
    public function setPlaylistUrlResolver($playlistFilename, $tvsMapItem,$sessionId)

    {

        return route('tvs-video.playlist', ['tvsMapItemId' => $tvsMapItem->id]) . '?info=' . $playlistFilename.'&session='.$sessionId;
    ...
    public function catchUpdateAdminEvent($table, object $oldĐata, object $newData)

    {

        $oldVideoIds = $this->getVideoIdFromDataObj($oldĐata);
...

    {
        ...
        foreach ($listVideoId as $item) {
            ...
            TvsMapItem::where('table_name', $table)->where('target_id', $targetId)->where('field', $item['field'])->delete();
...

    {
        ...
        foreach ($listVideoId as $item) {
            ...
            if (TvsMapItem::where('table_name', $table)->where('target_id', $targetId)->where('video_media_map_id', $item['id'])->where('field', $item['field'])->first() != null) {
