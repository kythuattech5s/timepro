...
namespace Tech5s\Voucher\Services;

use Tech5s\Voucher\Conditions\VoucherCondition;
...
class VoucherService extends VoucherCondition
{
    ...
    {
        $this->items = session()->get(static::PREFIX_SESSION_PRODUCT, collect());
        $this->categories = session()->get(static::PREFIX_SESSION_PRODUCT_CATEGORY, collect());
    ...
    public function setDataVoucher()
    {
        ...
        $this->setCategory($request->input('list_category', []));
    }
    public function setCategory($list)
    {
        if (is_string($list) && strlen($list) > 0) {
            $this->categories = collect(json_decode($list, true));
        }
    }
