...
namespace Tech5s\Voucher\Providers;
...
{
    const CONFIG_KEY_START = 'tpvc_';
    const CONFIG_ADMIN_KEY = 'sys_';

    public function boot()
    {
        $this->initConfigFile();
        $this->loadViewsFrom(__DIR__ . '/../../resources/views', 'tpv');
    ...
    public function initRouters()
    {
        ...
        $this->routes(function () use ($routes) {
            foreach ($routes as $route) {
                ...
                Route::prefix(config('tpvc_setting.route') . '/' . $name)->middleware('web')
                    ->namespace(config('tpvc_setting.controller'))
                    ->group($route);
            }
        });
    }
