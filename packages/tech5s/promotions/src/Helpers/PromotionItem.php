...
class PromotionItem
{
    ...
    public function getPriceCurrent($item)
    {
        ...
        // Thêm discount cho phần danh mục
        if ($item->catelog_price_rules_current->count() > 0) {
            foreach ($item->catelog_price_rules_current->sortByDesc('priority') as $catelog) {
                if (!isset($discount['by_catalog'])) {
                    $discount['by_catalog'] = collect([
                        'name' => 'Chiết khấu theo điều kiện danh mục',
                        'discount' => 0,
                    ]);
                }
                
                if (is_null($catelog->apply_next_rule) || (int) $catelog->apply_next_rule === static::NO_ACCEPT_NEXT_RULES || (isset($catelog->pivot->qty_apply)) && $this->qty < $catelog->pivot->qty_apply) {
                    break;
                };

                $qtyDiscount = $this->qty;

                if (isset($catelog->qty_apply)) {
                    $qtyDiscount = ($type->id == ProductHelper::TYPE_CLEAR_STOCK && $item->is_clear_stock == ProductHelper::IS_CLEAR_STOCK) ? floor($qtyDiscount / $catelog->pivot->qty_apply) : 0;
                }
                $discount_catalog = $catelog->apply_catalog_price_rule_method_id == static::RULES_METHOD_PERCENT ? $price * $catelog->discount_amount / 100 : $catelog->discount_amount;
                $price -= $discount_catalog;
                $discount['by_catalog']['discount'] += $discount_catalog * $qtyDiscount;
            }
        }
