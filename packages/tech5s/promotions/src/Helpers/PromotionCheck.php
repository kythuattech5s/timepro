...
use Tech5s\Promotion\Models\PromotionOnTotalInvoice;
use Tech5s\Promotion\Models\PromotionPriority;
use Auth;
...
class PromotionCheck
{
    ...
    public function __construct(array $listProduct = [], $noGetOldData = true)
    {
        if($noGetOldData){
            ...
            $this->getContentCart();
            $this->getDiscountByTotalInvoice();
    ...
    public function getDiscountByTotalInvoice()
    {
        foreach ($this->items as $item) {
            foreach ($item->discount as $type => $discount) {
                if (!isset($this->listDiscount[$type])) {
                    $this->listDiscount[$type] = collect();
                }
                if (!isset($this->listDiscount[$type]['discount'])) {
                    $this->listDiscount[$type]['discount'] = 0;
                }
                if (!isset($this->listDiscount[$type]['name'])) {
                    $this->listDiscount[$type]['name'] = $discount['name'];
                }
                $this->listDiscount[$type]['discount'] += $discount['discount'];
                $this->discoundFixed += $discount['discount'];
            }
            $this->totalPriceOrder += $item->qty * $item->product->price;
        }
        
        $priceAfterApplyFixedDiscount = $this->totalPriceOrder - $this->discoundFixed;

        $promotionSatisfyCoditionOnTotalInvoice = PromotionOnTotalInvoice::where('from_price', '<=', $priceAfterApplyFixedDiscount)->orderBy('from_price', 'DESC')->where('act', 1)->first();

        if ($promotionSatisfyCoditionOnTotalInvoice !== null) {
            $this->listDiscount['on_total_invoice'] = collect([
                'name' => 'Chiết khấu theo giá trị đơn hàng',
                'discount' => $promotionSatisfyCoditionOnTotalInvoice->type_discount == PromotionOnTotalInvoice::TYPE_PERCENT ? $priceAfterApplyFixedDiscount * $promotionSatisfyCoditionOnTotalInvoice->discount / 100 : $promotionSatisfyCoditionOnTotalInvoice->discount,
            ]);
        };
    }
