...
class DealCondition{
    ...
    public function checkTimeEditNow(){
        $request = request();
        $deal_start_at = clone $this->deal->start_at;
        $deal_expired_at = clone $this->deal->expired_at;
        $start_at = clone new DateTime($request->start_at);
        $expired_at = clone new DateTime($request->expired_at);
        $now = new DateTime();
