    ...
    <div class="deal-type">
        <div class="deal-type__item position-relative">
            ...
            <label for="type_code_shop" class="flex items-center">
                <img src="{{ asset('tech5sMarketing/images/shops.png') }}" class="mr-2" alt="Shop">
                Mua Kèm Deal Sốc
            </label>
        </div>
        <div class="deal-type__item position-relative">
            ...
            <label for="type_code_product" class="flex items-center">
                <img src="{{ asset('tech5sMarketing/images/online-shopping.png') }}" class="mr-2" alt="Shop">
               Mua Để Nhận Quà
            </label>
        </div>
    </div>
...
<div class="deal-group__form" data-type="{{ DealHelper::TYPE_GIFT }}" style="{{$currentItem->type == DealHelper::TYPE_GIFT ? '' : 'display:none'}}">
    ...
    <div class="form-gift d-flex align-items-center">
        ...
        <div class="group-input">
            <span>₫</span>
            <input type="text" name="price" value="{{$currentItem->price}}" rules="required" {{$currentItem->type == DealHelper::TYPE_GIFT ? '' : 'disabled'}}>
        </div>
        <p>để nhận</p>
...
<div class="deal-group__form justify-content-end">
    <button type="button" class="btn bg-green-400 text-white {{ $action == 'copy' ? 'save-deal' : 'save-edit-deal' }}" data-action="{{$action}}">Lưu</button>
</div>
