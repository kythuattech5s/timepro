...
use DateTime;
use Support;
use Tech5s\Promotion\Conditions\ComboCondition;
use Tech5s\Promotion\Old\OldDataCombo;

class ComboService extends ComboCondition{
    use OldDataCombo;
    public $combo;
    public $products;
    public $product_real;
    ...
    {
        $product_no_active = $this->products->filter(function($itemProduct){
            return !$this->product_real->first(function($product) use($itemProduct){
                return $product['id'] == $itemProduct['id'];
            });
        });
        foreach($product_no_active as $item){
    ...
    public function setDataProduct(){
        $listItems = $this->product_no_active->merge($this->product_real);
        foreach($listItems as $itemProduct){
    ...
    public function setDataCombo(){
        $request = request();
        $this->setAct(1);
        $this->setShop($request->input('shop_id'));
