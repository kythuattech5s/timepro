...
use DateTime;
use Tech5s\Promotion\Conditions\DealCondition;
...
class DealService extends DealCondition
{
    use OldDataDeal;
    public $deal;
    ...
    {
        $this->deal->shop_id = $shop_id ?? null;
    }
    ...
    public function setDataCreate()
    {
        ...
        $this->setShopId($request->input('shop_id'));
    ...
    {
        $this->product_mains = $this->product_mains->filter(function ($product) use ($id) {
    ...
    {
        $this->product_subs = $this->product_subs->filter(function ($product) use ($id) {
            return (int) $product['id'] !== (int) $id;
        });
    }
