...
class FlashSaleCondition extends BaseCodition
{
    ...
    public function checkTimeActiveOrCreateSatisfy()
    {
        ...
        $start_at = $this->flash_sale->start_at instanceof DateTime ? clone $this->flash_sale->start_at : clone new DateTime($this->flash_sale->start_at);
        //Lấy thời gian bắt đầu của phiên kết thúc hoặc thời gian của chương trình flash sale
        $expired_at = $this->flash_sale->expired_at instanceof DateTime ? clone $this->flash_sale->expired_at : clone new DateTime
         ...
         */
        // dd(FlashSale::where('act', 1)->where(function ($q) use ($start_at, $expired_at) {
        //     $q->where(function ($q) use ($start_at, $expired_at) {
        //         $q->whereBetween('start_at', [$start_at->modify('+1 minute'), $expired_at]);
        //     })->orWhere(function ($q) use ($start_at, $expired_at) {
        //         $q->whereBetween('expired_at', [$start_at->modify('+1 minute'), $expired_at->modify('+1 minute')]);
        //     });
        // })->get());
        // $flashsales = FlashSale::where('act', 1)->where(function ($q) use ($start_at, $expired_at) {
        //     $q->where(function ($q) use ($start_at, $expired_at) {
        //         $q->whereBetween('start_at', [$start_at->modify('+1 minute'), $expired_at->modify('+1 minute')]);
        //     })->orWhere(function ($q) use ($start_at, $expired_at) {
        //         $q->whereBetween('expired_at', [$start_at->modify('+1 minute'), $expired_at->modify('+1 minute')]);
        //     });
        // })->orWhere(function ($q) use ($start_at, $expired_at) {
        //     $q->where('start_at', '<=', $start_at->modify('+1 minute'))->where('expired_at', '>=', $expired_at->modify('+1 minute'));
        // });

        // if (isset($this->flash_sale->id)) {
        //     $flashsales->whereNotIn('id', [$this->flash_sale->id]);
        // }
        // $flashsales = $flashsales->count();
        // if ($flashsales > 0) {
    ...
    {
        $start_at = $this->flash_sale->start_at instanceof DateTime ? clone $this->flash_sale->start_at : clone new DateTime($this->flash_sale->start_at);
        $expired_at = $this->flash_sale->expired_at instanceof DateTime ? clone $this->flash_sale->expired_at : clone new DateTime($this->flash_sale->expired_at);
        $now = new DateTime();
        if ($expired_at < $now) {
