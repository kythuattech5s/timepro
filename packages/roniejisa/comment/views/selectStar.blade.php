<div class="rating-select" @if (isset($size) && $size) style="--font-size-select:{{ $size }}px" @endif>
    <div class="rating">
        @for ($i = 5; $i > 0; $i--)
            <input class="star star-{{ $i }}" rules="required" id="star-{{$name ?? 'rate'}}-{{ isset($keySelectStar) ? $keySelectStar . '-' : '' }}{{ $i }}" type="radio"
            ...
            <label class="star star-{{ $i }}" for="star-{{$name ?? 'rate'}}-{{ isset($keySelectStar) ? $keySelectStar . '-' : '' }}{{ $i }}"></label>
        @endfor
    </div>
</div>
