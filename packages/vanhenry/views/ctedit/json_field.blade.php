			...
			<?php foreach($subitems as $itemSubControl): ?>
				...
				<div class="elementor_json_field_control <?php echo (isset($itemSubControl['width'])&&$itemSubControl['width']==1)?'col-100':'col-50' ?>">
					...
					<div class="elementor_json_field_control_content col">
						@include('tv::ctedit.json_field.sub_edit_'.$typeSubControl,['itemSubControl'=>$itemSubControl,'field'=>$field,'key'=>$key,'subValue'=>$jsons,'nameSubControl'=>$nameSubControl])
					</div>
				</div>
			<?php endforeach; ?>
			<span class="close">
		...
		<?php foreach($jsons as $subValue): ?>
			...
			<?php foreach($subitems as $itemSubControl): ?>
				...
				<div class="elementor_json_field_control <?php echo (isset($itemSubControl['width'])&&$itemSubControl['width']==1)?'col-100':'col-50' ?> ">
					...
					<div class="elementor_json_field_control_content col">
						@include('tv::ctedit.json_field.sub_edit_'.$typeSubControl,['itemSubControl'=>$itemSubControl,'field'=>$field,'key'=>$key,'subValue'=>$subValue,'nameSubControl'=>$nameSubControl])
					</div>
				</div>
			<?php endforeach; ?>
			<span class="close">
