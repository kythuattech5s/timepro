...
@endphp
@if ($showAction)
    <a href="{{ url(FCHelper::ep($action, 'url')) . (count($params) > 0 ? '?' . implode('&', $params) : '') }}{{ (count($params) > 0 ? '&' : '?') . 'returnurl=' . $urlFull }}" @if (isset($action['target_blank']) && $action['target_blank']) target="_blank" @endif
