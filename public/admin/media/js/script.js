jQuery(function ($) {
    ...
    $(".media-item").on("click", ".mdi-btn .name-edit", function (event) {
        ...
        title.attr("data-old", title.text());
        title.children(".edit-ct").focus();
    });
    $(".media-item").on("blur", ".edit-ct", function () {
        if ($(this).val() != "" && $(this).val().trim() != "") {
            $(this).parent(".mdi-title").text($(this).attr("data-old"));
        }
        $(this).remove();
    });
    $(".media-item").on("keyup", ".edit-ct", function (e) {
        if (e.keyCode == 13) {
            const inputEl = $(this);
            const _this = this.closest(".mdi-title");
            if ($(this).val().trim() == "") return;
            const name = $(this).val();
            $.ajax({
                url: globalBaseUrl + "/media/rename",
                type: "POST",
                data: { id: $(this).attr("dt-id"), newname: name },
            }).done(function (data) {
                try {
                    if (data.code == 100) {
                        _this.innerHTML = _this.dataset.old;
                        $.simplyToast(data.message, "danger");
                    } else if (data.code == 200) {
                        _this.innerHTML = data.name;
