var XHR = (function () {
        ...
        var dataValues = Array.from(enableInputs).reduce(function (
            ...
            switch (input.type) {
                case "radio":
                    ...
                    break;
                case "checkbox":
                    if(!values[input.name] && (input.matches(":checked") || noCheckAndNoValue)){
    ...
    function buildFormData(data, token = []) {
    ...
    return {
        createFormData: function (form) {
            ...
            const data = getDataValues(inputs, true);
            return buildFormData(data);
        },
        buildData: function (data, getNoChecked = false) {
            return getDataValues(data, getNoChecked);
        },
        send: function (data) {
            return _send(data);
        },
