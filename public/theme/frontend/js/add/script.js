var MORE_FUNCTION = (function(){
	...
	var changeBirthDayProfile = function(element){
		...
		if(type == 'month'){
			document.querySelector('select[data-type="day"]').value = '0';
			var dayOld = document.querySelector('select[data-type="day"]').getAttribute('data-day');
			XHR.send({
				url: "get-last-dat-of-month",
				method: "POST",
				data:{
					year: document.querySelector('select[data-type="year"]').value,
					month: document.querySelector('select[data-type="month"]').value,
					day: dayOld,
				}
			})
			.then((res) => {
				document.querySelector('select[data-type="day"]').innerHTML = res.html;
			});
		}
		if(type == 'day'){
			var birday = document.querySelector('select[data-type="year"]').value + '-' + document.querySelector('select[data-type="month"]').value + '-' + _this.value;
	...
	var getDistrictByProvince = function(){
		...
		provinces.forEach(function(item){
			item.addEventListener('change',function(){
				var district_select = document.querySelector('select[district]');
				var district_value = district_select.getAttribute('data-value');
				XHR.send({
					url: "get-district-by-province?province_id="+this.value+'&district_id='+district_value,
					method: "GET"
				})
				.then((res) => {
	...
	var getWardByDistrict = function(){
		...
		provinces.forEach(function(item){
			item.addEventListener('change',function(){
				...
				XHR.send({
					url: "get-ward-by-district?district_id="+this.value+'&ward_id='+ward_value,
					method: "GET"
				})
				.then((res) => {
